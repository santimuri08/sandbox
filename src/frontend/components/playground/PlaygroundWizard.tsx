...
const HOST_OPTIONS: SelectOption[] = [
	...
	{ label: 'Local (Docker/File)', value: 'none' }
];
...
const warningBannerStyle: StyleObject = {
...
const hasAnyWorkingHost = (database: DatabaseEngine) => {
	const workingHosts = WORKING_COMBINATIONS[database] || [];

	return workingHosts.length > 0;
};
...
const WarningBanner = ({ children }: WarningBannerProps) => (
	<aside style={warningBannerStyle}>
...
const ProjectNameStep = ({ error, onNameChange, onValidate, projectName }: ProjectNameStepProps) => {
	...
	return (
		<section>
			<StepHeader
				description="Choose a name for your AbsoluteJS project"
				title="Project Name"
			/>
			<input
...
const CodeQualityStep = ({ onToolChange, selectedTool }: CodeQualityStepProps) => (
	<section>
		<StepHeader
			description="Choose your preferred linting and formatting setup"
		...
		<nav style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
			<OptionCard
				...
				subtitle="Industry standard combination"
			/>
			<OptionCard
				label="Biome"
				onClick={() => onToolChange('biome')}
				selected={selectedTool === 'biome'}
				subtitle="Fast, all-in-one toolchain"
			/>
		</nav>
	</section>
);
...
const TailwindStep = ({ onTailwindChange, tailwindEnabled }: TailwindStepProps) => (
	<section>
		<StepHeader
			description="Would you like to include Tailwind CSS for styling?"
			title="Tailwind CSS"
		/>
		<nav style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
			<OptionCard
			...
			<OptionCard
				...
				selected={!tailwindEnabled}
				subtitle="Use your own CSS solution"
			/>
		</nav>
	</section>
);
...
const FrontendsStep = ({ onFrontendsChange, selectedFrontends }: FrontendsStepProps) => {
	...
	return (
		<section>
			<StepHeader
				description="Select one or more frontend frameworks (at least one required)"
...
const DatabaseHostStep = ({ database, onHostChange, selectedHost }: DatabaseHostStepProps) => {
	const noWorkingHosts = !hasAnyWorkingHost(database);
	...
	return (
		<section>
			...
			{noWorkingHosts && (
				<WarningBanner>
					⚠️ <strong>{database}</strong> doesn&apos;t have cloud hosting options available in the playground yet.
					Local hosting requires Docker which isn&apos;t available in this trial environment.
					<br /><br />
					<strong>Tip:</strong> Try <strong>PostgreSQL + Neon</strong> or <strong>SQLite + Turso</strong> for a working setup,
					or download the generated project to run locally with Docker.
				</WarningBanner>
			)}
			<nav style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
...
const OrmStep = ({ onOrmChange, selectedOrm }: OrmStepProps) => (
	<section>
		<StepHeader
			description="Choose an ORM for database interactions"
			title="ORM Selection"
		/>
		<nav style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
...
const FolderStructureStep = ({ onConfigChange, selectedConfig }: FolderStructureStepProps) => (
	<section>
		<StepHeader
			description="Choose your project's folder structure"
...
const AuthenticationStep = ({ onAuthChange, selectedAuth }: AuthenticationStepProps) => (
	<section>
		<StepHeader
			description="Would you like to include authentication?"
...
const PluginsStep = ({ onPluginsChange, selectedPlugins }: PluginsStepProps) => {
	...
	return (
		<section>
			<StepHeader
				description="Select optional plugins to include (optional)"
				title="Plugins"
			/>
			<nav style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
...
const ReviewStep = ({
	...
	return (
		<section>
			<StepHeader
				description="Review your choices before generating the project"
