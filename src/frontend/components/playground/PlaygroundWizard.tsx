...
const HOST_OPTIONS = [
	...
	{ value: 'none', label: 'Local (Docker/File)'}
];
...
export function PlaygroundWizard({ themeSprings, onGenerate }: PlaygroundWizardProps) {
	...
	const requiresDockerForLocal = (db: DatabaseEngine): boolean => {
		return DATABASES_REQUIRING_DOCKER.includes(db);
	};
	const isHostWorkingForDatabase = (db: DatabaseEngine, host: string): boolean => {
		const workingHosts = WORKING_COMBINATIONS[db] || [];
		return workingHosts.includes(host);
	};
	const hasAnyWorkingHost = (db: DatabaseEngine): boolean => {
		const workingHosts = WORKING_COMBINATIONS[db] || [];
	...
	const renderStep = () => {
		...
		switch (stepName) {
			...
			case 'Database Host':
				const noWorkingHosts = !hasAnyWorkingHost(databaseEngine);
				return (
					<div>
						<h2 style={{ fontSize: '1.5rem', marginBottom: '0.5rem', color: '#fff' }}>
							Database Host
						...
						{noWorkingHosts && (
							<div style={warningBannerStyle}>
								<p style={{ color: '#f59e0b', margin: 0, fontSize: '0.875rem' }}>
									⚠️ <strong>{databaseEngine}</strong> doesn't have cloud hosting options available in the playground yet. 
						...
						<div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
							{HOST_OPTIONS.map(option => {
								...
								const isLocalOption = option.value === 'none';
								const showDockerWarning = isLocalOption && requiresDockerForLocal(databaseEngine);				
								return (
									<div
										key={option.value}
										onClick={() => {
									...
									>
										...
										{!isWorking && !isLocalOption && (
											<div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.75rem', marginTop: '0.5rem' }}>
												Not available for {databaseEngine} yet
											</div>
										)}
									</div>
								);
