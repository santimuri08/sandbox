/**
 * PlaygroundWizard.tsx
 *
 * WHERE IT GOES:
 * src/frontend/components/playground/PlaygroundWizard.tsx
 * 
 * QUESTION ORDER (matches CLI prompt.ts):
 * 1. Project Name
 * 2. Code Quality (eslint+prettier or biome)
 * 3. Tailwind CSS
 * 4. Frontend Frameworks
 * 5. HTML Scripts (conditional - only if HTML selected)
 * 6. Database Engine
 * 7. Database Host (for all databases except 'none')
 * 8. ORM (conditional - only for SQL databases)
 * 9. Folder Structure
 * 10. Authentication
 * 11. Plugins
 * 12. Review
 * 
 * NOTE: Git and Install are NOT asked - we skip git and always install with Bun
 */
...
// All hosting options available for all databases
// Status: working = ✅, coming = 🔜
const HOST_OPTIONS = [
...
// Which database + host combinations are currently working
// All others will show "Coming Soon" badge
const WORKING_COMBINATIONS: Record<string, string[]> = {
	postgresql: ['neon', 'planetscale', 'none'],
	sqlite: ['turso', 'none'],
	mysql: ['none'],
	mariadb: ['none'],
	mongodb: ['none'],
...
// Databases that support ORMs (SQL databases only, not MongoDB/Gel)
...
export function PlaygroundWizard({ themeSprings, onGenerate }: PlaygroundWizardProps) {
	...
	// Form state - in CLI question order
	...
	// Get default host for a database (first working option)
	const getDefaultHostForDatabase = (db: DatabaseEngine): DatabaseHost => {
		const workingHosts = WORKING_COMBINATIONS[db] || ['none'];
		// Return first cloud option if available, otherwise 'none'
		const cloudOption = workingHosts.find(h => h !== 'none');
		return (cloudOption || 'none') as DatabaseHost;
	};

	// Calculate which steps to show dynamically based on selections
	const getSteps = () => {
		const steps = [
			'Project Name',        // 1
			'Code Quality',        // 2
		...
		// 5. HTML Scripts - only if HTML frontend selected
		if (hasHtmlFrontend()) {
			steps.push('HTML Scripts');
		}

		steps.push('Database'); // 6
		...
		// 8. ORM - only for SQL databases (not MongoDB/Gel)
		if (databaseEngine !== 'none' && supportsOrm(databaseEngine)) {
			steps.push('ORM');
		}

		steps.push('Folder Structure'); // 9
		steps.push('Authentication');   // 10
		steps.push('Plugins');          // 11
		steps.push('Review');           // 12
	...
	const canProceed = (): boolean => {
		...
		switch (stepName) {
			...
			case 'Database Host':
				// Must select a working combination
	...
	const handleNext = () => {
		...
		// Reset dependent values when database changes
		if (stepName === 'Database') {
			setDatabaseHost(getDefaultHostForDatabase(databaseEngine));
		}

		// Reset HTML scripts if HTML is deselected
	...
	const handleGenerate = () => {
		const config: PlaygroundConfig = {
			...
			authProvider,
			selectedPlugins,
			gitInit: false,      // Always skip git
			installDeps: true    // Always install with Bun
		};
					...
					<div>
						...
						<div style={{
							...
							<p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.875rem', margin: 0 }}>
								⏱️ Dependencies will be installed automatically. This may take 2-5 minutes.
							</p>
						</div>
					</div>
				);
