...
const generatePackageJson = (config: PlaygroundConfig) => {
	...
	if (config.selectedPlugins.includes('@elysiajs/cors')) {
		deps['@elysiajs/cors'] = '^1.2.0';
	}

	if (config.selectedPlugins.includes('@elysiajs/swagger')) {
		deps['@elysiajs/swagger'] = '^1.2.0';
	}
	...
	return JSON.stringify({
		...
		scripts: {
			'bun:dev': 'bun run --hot src/backend/server.ts',
			dev: 'node server.js',
...
const getThemeForFrontend = (frontends: string[]) => {
	if (frontends.includes('react')) {
		return { primary: '#61dafb', secondary: '#282c34' };
	}

	if (frontends.includes('vue')) {
		return { primary: '#42b883', secondary: '#35495e' };
	}

	if (frontends.includes('svelte')) {
		return { primary: '#ff3e00', secondary: '#40b3ff' };
	}
...
const generateServerFile = (config: PlaygroundConfig) => {
	const { primary, secondary } = getThemeForFrontend(config.frontends);
	...
	const techBadges = features
		.map((feature) => `<span class="tech-badge">${feature}</span>`)
		.join('\n\t\t');
...
const generateElysiaServer = (config: PlaygroundConfig) => {
	...
	if (plugins.includes('@elysiajs/cors')) {
		imports.push("import { cors } from '@elysiajs/cors';");
		uses.push('.use(cors())');
	}

	if (plugins.includes('@elysiajs/swagger')) {
...
const generateReadme = (config: PlaygroundConfig) => {
	...
	const tailwindInfo = config.useTailwind ? '- Tailwind CSS\n' : '';
...
const generateEnv = (config: PlaygroundConfig) => {
	let env = 'PORT=3000\n';
...
const generateReactPage = (config: PlaygroundConfig) =>
