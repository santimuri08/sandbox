...
function buildCliArgs(config: PlaygroundConfig): string[] {
	...
	// Always add --install to install dependencies
	args.push('--install');
...
async function runCreateAbsoluteJS(
	...
	return new Promise((resolve) => {
		...
		// Answer ALL possible prompts automatically:
		// The CLI may ask several questions even with --skip flag
		// We send answers with delays to handle each prompt as it appears
		const answers = [
			{ delay: 500, answer: 'n\n' },   // HTML scripts: No (or first prompt)
			{ delay: 1000, answer: 'n\n' },  // Git initialization: No
			{ delay: 1500, answer: 'y\n' },  // Install dependencies: Yes
			{ delay: 2000, answer: 'n\n' },  // Any additional prompt: No
			{ delay: 2500, answer: 'y\n' },  // Any additional prompt: Yes
		];
